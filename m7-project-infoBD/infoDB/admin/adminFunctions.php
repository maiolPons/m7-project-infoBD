                ...
                if(comprovarImatge($_FILES["foto"])){
                    ...
                    if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                        if(crearProfesor($_POST,$newName)){
                            ...
                            echo '<META HTTP-EQUIV="REFRESH" CONTENT="1;URL=adminMenu.php?opcio=crear">';
                        }
                        else{
    ...
    if(isset($_GET['modificarDni'])){
        ...
        if($consulta == false){
            ...
            echo' <form action="adminmenu.php" method="POST" enctype="multipart/form-data">';
                ...
                echo'<input placeholder='.$registre["dni"].' type="text" name="dni" require><br>';
                echo'<label for="contrasenya">Contrasenya:</label><br>';
                echo'<input placeholder='.$registre["contrasenya"].' type="password" name="contrasenya" require><br><br>';
                echo'<label for="nom">Nom:</label><br>';
                echo'<input placeholder='.$registre["nom"].' type="text" name="nom" require><br><br>';
                echo'<label for="cognoms">Cognoms:</label><br>';
                echo'<input placeholder='.$registre["cognom"].' type="text" name="cognoms" require><br><br>';
                echo'<label for="text">Titol academic</label><br>';
                echo'<input placeholder='.$registre["titolAcademic"].' type="titol" name="titol" require><br><br>';
        ...
        if(!empty($_POST["codi"]) && !empty($_POST["nom"]) && !empty($_POST["descripcio"]) && !empty($_POST["hores"]) && !empty($_POST["dataInici"])&& !empty($_POST["dataFinal"])){
            if(comprobardni('cursos',$_POST["codi"])){
                if(crearcurs($_POST)){
                    ...
                    echo '<META HTTP-EQUIV="REFRESH" CONTENT="1;URL=adminMenu.php?opcio=llistar">';
    ...
    if(isset($_GET['modificarCodi'])){
        ...
        if($consulta == false and $consulta2 == false){
            ...
            echo' <form action="adminmenu.php" method="POST" enctype="multipart/form-data">';
                ...
                echo'<input placeholder='.$registre["codi"].' type="text" name="codi" require><br>';
                echo'<label for="nom">Nom:</label><br>';
                echo'<input placeholder='.$registre["nom"].' type="text" name="nom" require><br><br>';
                echo'<label for="descripcio">Descripcio :</label><br>';
                echo'<input placeholder='.$registre["descripcio"].' type="textarea" name="descripcio" require><br><br>';
                echo'<label for="hores">Hores:</label><br>';
                echo'<input placeholder='.$registre["hores"].' type="text" name="hores" require><br><br>';
                echo'<label for="text">Data Inici</label><br>';
                echo'<input placeholder='.$registre["dataInici"].' type="date" name="dataInici" require><br><br>';
                echo'<label for="text">Data Final</label><br>';
                echo'<input placeholder='.$registre["dataFinal"].' type="date" name="dataFinal" require><br><br>';
    ...
    if(isset($_POST["modificarcurs"])){
        if(!empty($_POST["codi"]) && !empty($_POST["nom"]) && !empty($_POST["descripcio"]) && (!empty($_POST["hores"]) || $_POST["hores"]==0) && !empty($_POST["dataInici"])&& !empty($_POST["dataFinal"])){
            if(comprobardniModificar('cursos',$_POST["codi"],$_POST["oldCodi"])){
                ...
                echo "<p class='warning'>Codi invalid!</p>";
            }
        }
        else{
            ...
            if($_SESSION["menu"]=="Profesorat"){
                ...
                echo "<div class='contenidorTaula2'>";
                    ...
                    while($row = mysqli_fetch_array($consulta)){
                        ...
                        if(isset($_POST["opcio"])){
                            if($_POST["opcio"]=="modificar"){
                                echo '<td><a href="adminMenu.php?menu=' . $row["dni"] . '">Modificar</a></td>';
                            }
                            if($_POST["opcio"]=="eliminar"){
                                echo '<td><a href="adminMenu.php?menu=' .$row["fotografia"] . '">Eliminar</a></td>';
                            }
                        }
